...
void Mits88Disk::attachFile(int drive, char const * filename)
{
  ...
  if (stat(filename, &st) != 0) {
    ...
    for (int i = 0; i < TRACK_SIZE * TRACKS_COUNT; ++i)
      fwrite(sector, sizeof(sector), 1, m_file[drive]);
    fflush(m_file[drive]);
  } else {
...
int Mits88Disk::readByteFromDisk(int position)
{
  if (m_readOnlyBuffer[m_drive]) {
    return m_readOnlyBuffer[m_drive][position];
  } else if (m_file[m_drive])
    return fgetc(m_file[m_drive]);
  else
    return 0xFF;
}


void Mits88Disk::writeByteToDisk(int position, int value)
{
  if (m_readOnlyBuffer[m_drive]) {
    // this is read only!
  } else if (m_file[m_drive]) {
    fseek(m_file[m_drive], position, SEEK_SET);
    fputc(value, m_file[m_drive]);
  }
}
...
bool Mits88Disk::read(int address, int * result)
{
  ...
  switch (address) {
    ...
    case 0x0A:
      // read data
      *result = readByteFromDisk(TRACK_SIZE * SECTOR_SIZE * m_track[m_drive] + m_sector[m_drive] * SECTOR_SIZE + m_pos[m_drive]);
...
bool Mits88Disk::write(int address, int value)
{
  ...
  switch (address) {
    ...
    case 0x0A:
      // write data
      if (m_pos[m_drive] == SECTOR_SIZE) {
        // writing 138th byte after the end of sector...ignore and flush
        m_pos[m_drive] = 0;
        flushDisk();
      } else {
        writeByteToDisk(TRACK_SIZE * SECTOR_SIZE * m_track[m_drive] + m_sector[m_drive] * SECTOR_SIZE + m_pos[m_drive], value);
        ++m_pos[m_drive];
      }
      return true;
