...
{
  if (bufferedFileChanged && file && bufferedFileDataPos != -1) {
    suspendInterrupts();
...
IRAM_ATTR void Machine::run(CPU cpu, int address)
{
  if (cpu == i8080) {
    ...
    while (true) {
      if (m_realSpeed) {
        int64_t t = esp_timer_get_time();  // time in microseconds
        int cycles = Z80Emulate(&Z80State, 0, this);
        if (m_realSpeed) {
          t += cycles / 2;    // at 2MHz each cycle last 0.5us, so instruction time is cycles*0.5, that is cycles/2
          while (esp_timer_get_time() < t)
            ;
        ...
        Z80Emulate(&Z80State, 0, this);
        if (Keyboard.isVKDown(VirtualKey::VK_F12))
          emulator_menu();
      }
    }
