    ...
    *m_destScanline++ = m_borderColor4;
    ...
    if (~(m_leftPos + x) & 0x4) {
      ...
      if (m_foregroundColorCode & 0x8) {
        ...
        // pack 4 pixels in raw 32 bit word and draw right now
        *m_destScanline++ =  (m_mcolors[(cv >> 6) & 3] << 16)
                           | (m_mcolors[(cv >> 6) & 3] << 24)
                           | (m_mcolors[(cv >> 4) & 3]      )
                           | (m_mcolors[(cv >> 4) & 3] << 8 );

        // pack low 4 pixels in raw 32 bit word
        m_foreGround4     =  (m_mcolors[(cv >> 2) & 3] << 16)
                           | (m_mcolors[(cv >> 2) & 3] << 24)
                           | (m_mcolors[(cv     ) & 3]      )
                           | (m_mcolors[(cv     ) & 3] << 8 );
        ...
        // pack high 4 pixels in raw 32 bit word and draw right now
        *m_destScanline++ =  (m_hcolors[(cv >> 7) & 1] << 16)
                           | (m_hcolors[(cv >> 6) & 1] << 24)
                           | (m_hcolors[(cv >> 5) & 1]      )
                           | (m_hcolors[(cv >> 4) & 1] << 8 );

        // pack low 4 pixels in raw 32 bit word
        m_foreGround4     =  (m_hcolors[(cv >> 3) & 1] << 16)
                           | (m_hcolors[(cv >> 2) & 1] << 24)
                           | (m_hcolors[(cv >> 1) & 1]      )
                           | (m_hcolors[(cv)      & 1] << 8 );
      ...
      // draw the other nibble
      *m_destScanline++ = m_foreGround4;
...
void MOS6561::writeReg(int reg, int value)
{
  if (m_regs[reg] != value) {
    ...
    switch (reg) {
      case 0x0:
        // in real system m_leftPos can be negative. Due optimizations in drawNextPixels() we have to limit it!
        m_leftPos  = fabgl::imax(0, ((m_regs[0] & 0x7f) - 5) * 4);
      ...
      case 0xa:
        ...
        //m_sqGen1.setFrequency(PHI2 / 4 / 256 / (128 - ((value + 1) & 0x7f)));
        m_sqGen1.setFrequency(PHI2 / 4 / 256 / (127 - (value & 0x7f)));//...division by zero check?
        break;
      case 0xb:
        m_sqGen2.enable(value & 0x80);
        //m_sqGen2.setFrequency(PHI2 / 4 / 128 / (128 - ((value + 1) & 0x7f)));
        m_sqGen2.setFrequency(PHI2 / 4 / 128 / (127 - (value & 0x7f)));
        break;
      case 0xc:
        m_sqGen3.enable(value & 0x80);
        //m_sqGen3.setFrequency(PHI2 / 4 / 64 / (128 - ((value + 1) & 0x7f)));
        m_sqGen3.setFrequency(PHI2 / 4 / 64 / (127 - (value & 0x7f)));
        break;
      case 0xd:
        m_noGen.enable(value & 0x80);
        m_noGen.setFrequency(PHI2 / 4 / 32 / (127 - (value & 0x7f)));
        break;
      case 0xf:
      {
