...
void MOS6561::drawNextPixels()
{
  ...
  if (y < m_topPos || y >= m_topPos + charAreaHeight) {
    ...
    if (x < m_leftPos || x >= m_leftPos + m_charAreaWidth) {
      for (int i = 0; i < 4; ++i)
        SETPIXEL(x + i, y, borderColor);
      return;
    }
    ...
    if (foregroundColorCode & 0b1000) {
      ...
      const RGB colors[4] = { m_backgroundColor, borderColor, foregroundColor, auxColor };
