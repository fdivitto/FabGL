...
#define READ(addr) Read(Context, (addr))
#define WRITE(addr, value) Write(Context, (addr), (value))


mos6502::mos6502(BusRead r, BusWrite w, void * context)
{
	Write = (BusWrite)w;
	Read = (BusRead)r;
  Context = context;
...
int mos6502::Run()
{
  Instr instr = InstrTable[READ(pc++)];
  Exec(instr);
