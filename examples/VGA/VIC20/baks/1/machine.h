...
public:
  ...
  void tick(uint32_t cycle);

  bool interrupt()                   { return m_regs[VIA_REG_IER] & m_regs[VIA_REG_IFR] & 0x7f; }
...
private:
  Machine *  m_machine;
  uint16_t   m_timer1Counter;
  uint16_t   m_timer1Precount;
  uint16_t   m_timer1Latch;
  uint16_t   m_timer2Counter;
  uint8_t    m_regs[16];
...
private:
  ...
  int charTableAddress_VIC2CPU(int addr);
  void drawNextPixels();
...
private:
  ...
  static uint8_t busRead(void * context, uint16_t addr);
  static void busWrite(void * context, uint16_t addr, uint8_t value);
