...
Machine::Machine()
  : m_CPU(&Machine::busRead, &Machine::busWrite, this),
...
{
  #if DEBUGMSG
  printf("exec 0x%04x 0x%02x, ", m_CPU.PC(), busRead(m_CPU.PC()));
  printf("VIA1 "); m_VIA1.dump();
  printf(", VIA2 "); m_VIA2.dump();
  printf("\n");
  #endif
  ...
  for (int c = 0; c < cycles; ++c, ++m_cycle) {
...
/*
    // VIC
    bool screenRefresh = m_VIC.tick(m_cycle);
...
uint8_t Machine::busRead(void * context, uint16_t addr)
{
  return ((Machine *)context)->busRead(addr);
}


void Machine::busWrite(void * context, uint16_t addr, uint8_t value)
{
  ((Machine *)context)->busWrite(addr, value);
}
...
void MOS6522::writeReg(int reg, int value)
{
  ...
  switch (reg) {
    ...
    case VIA_REG_T2_C_HI:
      ...
      m_regs[VIA_REG_IFR] &= ~VIA_I_T2;
      m_timer2Triggered = false;
      break;
    case VIA_REG_ACR:
      m_regs[VIA_REG_ACR] = value;
      break;
    case VIA_REG_PCR:
    {
    ...
    case VIA_REG_ORA:
      ...
      m_regs[VIA_REG_IFR] &= ~VIA_I_CA1;
      m_regs[VIA_REG_IFR] &= ~VIA_I_CA2;
      break;
    case VIA_REG_ORA_NH:
    ...
    case VIA_REG_ORB:
      ...
      m_regs[VIA_REG_IFR] &= ~VIA_I_CB1;
      m_regs[VIA_REG_IFR] &= ~VIA_I_CB2;
      break;
    default:
      break;
  };
...
int MOS6522::readReg(int reg)
{
  ...
  switch (reg) {
    ...
    case VIA_REG_ACR:
      return m_regs[VIA_REG_ACR];
    case VIA_REG_PCR:
      return m_regs[VIA_REG_PCR];
    case VIA_REG_IER:
      return m_regs[VIA_REG_IER] | 0x80;
    case VIA_REG_IFR:
      return m_regs[VIA_REG_IFR] | (m_regs[VIA_REG_IFR] & m_regs[VIA_REG_IER] ? 0x80 : 0);
    case VIA_REG_DDRA:
      // Direction register Port A
...
void MOS6522::tick(uint32_t cycle)
{
  // handle Timer 1
  if (m_timer1Precount > 0) {
    ...
    --m_timer1Counter;
    if (!m_timer1Triggered && m_timer1Counter == 0) {
      if (m_regs[VIA_REG_ACR] & VIA_ACR_T1_FREERUN) {
        // free run, reload from latch
        m_timer1Precount = 2;                   // +2 delay before next start
        m_timer1Counter  = m_timer1Latch;
        #if DEBUGMSG
        printf("VIA %d timer 1 - free run - latch %d\n", m_tag, m_timer1Latch);
        #endif
      } else {
        // one shot
        m_timer1Triggered = true;
        #if DEBUGMSG
        printf("VIA %d timer 1 - one shot - latch %d\n", m_tag, m_timer1Latch);
        #endif
      }
      // set interrupt flag
      m_regs[VIA_REG_IFR] |= VIA_I_T1;
  ...
  // handle Timer 2
  if ((m_regs[VIA_REG_ACR] & VIA_ACR_T2_COUNTPULSES) == 0)
    --m_timer2Counter;
  if (!m_timer2Triggered && m_timer2Counter == 0) {
    m_timer2Triggered = true;
    // set interrupt flag
    m_regs[VIA_REG_IFR] |= VIA_I_T2;
    #if DEBUGMSG
    printf("VIA %d timer 2- latch %d\n", m_tag, m_timer2Latch);
    #endif
  }
...
/*
bool MOS6522::interrupt()
{
  return m_regs[VIA_REG_IER] & m_regs[VIA_REG_IFR] & 0x7f;
}
*/
...
bool MOS6561::tick(uint32_t cycle)
{
...
//#define SETPIXEL(x, y, value) m_frameBuffer[(y) * ScreenWidth + (x)] = (value)
/*
...
void MOS6561::drawNextPixels()
{
  ...
  int topPos         = (m_regs[1] - 14) * 2;
  int charAreaHeight = m_rowCount * m_charHeight;

  RGB borderColor = CHARCOLORS[m_regs[0xf] & 7];

  if (y >= topPos && y < topPos + charAreaHeight) {

    int leftPos       = ((m_regs[0] & 0x7f) - 5) * 4;
    int charAreaWidth = m_colCount * CharWidth;

    // left or right border?
    if (x < leftPos || x >= leftPos + charAreaWidth) {
      for (int i = 0; i < 4; ++i)
        SETPIXEL(x + i, y, borderColor);
      return;
    }
    ...
    RGB backgroundColor = backColorCode < 8 ? CHARCOLORS[backColorCode] : AUXCOLORS[backColorCode & 7];

    int charRow = (y - topPos) / m_charHeight;
    int charCol = (x - leftPos) / CharWidth;

    int charIndex = m_machine->busRead(m_videoMatrixAddr + charRow * m_colCount + charCol);

    // character color code from color RAM
    int foregroundColorCode = m_machine->busRead(m_colorMatrixAddr + charRow * m_colCount + charCol);
    RGB foregroundColor = CHARCOLORS[foregroundColorCode & 7];

    int cy = (y - topPos) % m_charHeight;
    int cv = m_machine->busRead(charTableAddress_VIC2CPU(charIndex * m_charHeight + cy));

    int startBit = ((leftPos + x) & 0x4) ? 3 : 7;

    if (foregroundColorCode & 0b1000) {
      ...
      const RGB colors[4] = { backgroundColor, borderColor, foregroundColor, auxColor };

      for (int i = 0; i < 4; i += 2) {
        int code = (cv >> (startBit - 1 - i)) & 0b11;
      ...
      if ((m_regs[0xf] & 0b1000) == 0) {
        RGB t = foregroundColor;
        foregroundColor = backgroundColor;
        backgroundColor = t;
      }

      for (int i = 0; i < 4; ++i)
        SETPIXEL(x + i, y, cv & (1 << (startBit - i)) ? foregroundColor : backgroundColor);
