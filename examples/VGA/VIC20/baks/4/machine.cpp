...
void MOS6561::tick(int cycles)
{
  for (int c = 0; c < cycles; ++c) {
    ...
    if (m_videoLine && m_scanX >= HorizontalBlanking)
      drawNextPixels();
  }
}


#define SETPIXEL(x, rgb) PIXELINROW(m_destScanline, x) = 0xc0 | *((uint8_t*)&(rgb));
...
void MOS6561::drawNextPixels()
{
  ...
  if (m_isVBorder || x < m_leftPos || x >= m_leftPos + m_charAreaWidth) {
    ...
    SETPIXEL(x,     m_borderColor);
    SETPIXEL(x + 1, m_borderColor);
    SETPIXEL(x + 2, m_borderColor);
    SETPIXEL(x + 3, m_borderColor);
    ...
    //int charIndex = m_machine->vicBusRead(m_videoMatrixAddr + charRow * m_colCount + charCol);
    int charIndex = m_videoLine[charCol];

    // character color code from color RAM
    int foregroundColorCode = m_colorLine[charCol];
    RGB foregroundColor = CHARCOLORS[foregroundColorCode & 7];

    int cv = m_machine->vicBusRead(charTableAddress_VIC2CPU(charIndex * m_charHeight + m_inCharRow));
    ...
    if (foregroundColorCode & 0x8) {
      ...
      const RGB colors[4] = { m_backgroundColor, m_borderColor, foregroundColor, m_auxColor };

      for (int i = 0; i < 4; i += 2) {
        int code = (cv >> (startBit - 1 - i)) & 0x3;
        SETPIXEL(x + i + 0, colors[code]);
        SETPIXEL(x + i + 1, colors[code]);
      }
      ...
      for (int i = 0; i < 4; ++i)
        SETPIXEL(x + i, cv & (1 << (startBit - i)) ? fColor : bColor);
  ...
  if (m_regs[reg] != value) {
    ...
    switch (reg) {
      ...
      case 0xe:
      {
        ...
        m_auxColor       = auxColorCode < 8 ? CHARCOLORS[auxColorCode] : AUXCOLORS[auxColorCode & 7];
        break;
      }
      case 0xf:
      {
