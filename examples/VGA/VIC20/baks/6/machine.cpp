...
void MOS6561::drawNextPixels()
{
  ...
  if (m_isVBorder || x < m_leftPos || x >= m_leftPos + m_charAreaWidth) {
    ...
    uint8_t charStart = ~(m_leftPos + x) & 0x4;
    ...
    if (charStart) {
      ...
      m_foregroundColorCode = m_colorLine[charColumn];

      // character pixels from char table
      m_charData = m_machine->vicBusRead(CHARTABLE_VIC2CPU(m_charTableAddr + charIndex * m_charHeight + m_inCharRow));

      m_mcolors[2] = m_hcolors[1] = RAWCOLORS[m_foregroundColorCode & 7]; // foreground color
    }

    // select nibble to draw
    uint8_t cv = m_charData >> charStart;
    ...
    if (m_foregroundColorCode & 0x8) {
      ...
      uint32_t px =   (m_mcolors[(cv >> 2) & 3] << 16)
                    | (m_mcolors[(cv >> 2) & 3] << 24)
                    | (m_mcolors[(cv >> 0) & 3]      )
                    | (m_mcolors[(cv >> 0) & 3] << 8 );
      SETPIXEL4(x, px);
      ...
      // packs 4 pixels in raw 32 bit word
      uint32_t px =   (m_hcolors[(cv >> 3) & 1] << 16)
                    | (m_hcolors[(cv >> 2) & 1] << 24)
                    | (m_hcolors[(cv >> 1) & 1]      )
                    | (m_hcolors[(cv)      & 1] << 8 );
      SETPIXEL4(x, px);
