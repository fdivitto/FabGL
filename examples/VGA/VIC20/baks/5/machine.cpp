...
// only addresses restricted to 6561
uint8_t const * Machine::vicBusReadP(uint16_t addr)
{
  int addr_hi = (addr >> 8) & 0xff; // 256B blocks
  int block   = (addr >> 12) & 0xf; // 4K blocks
...
void MOS6561::tick(int cycles)
{
  for (int c = 0; c < cycles; ++c) {
    ...
    if (m_scanX == FrameWidth) {
      ...
      if (m_scanY >= VerticalBlanking) {
        m_Y            = m_scanY - VerticalBlanking;
        m_destScanline = VGAController.getScanline(m_Y);
        ...
        if (!m_isVBorder) {
          ...
          m_videoLine = m_machine->vicBusReadP(m_videoMatrixAddr + m_charRow * m_colCount);
          m_colorLine = m_machine->vicBusReadP(m_colorMatrixAddr + m_charRow * m_colCount);
        }
      } else {
        m_isVBorder = false;
        m_videoLine = nullptr;
...
void MOS6561::drawNextPixels()
{
  ...
  if (m_isVBorder || x < m_leftPos || x >= m_leftPos + m_charAreaWidth) {
    ...
    SETPIXEL4(x, m_borderColor4);

  } else {

    // chars area

    int charColumn = (x - m_leftPos) >> 3;  // assume CharWidth = 8 (should be "(x - m_leftPos) / CharWidth")

    int charIndex = m_videoLine[charColumn];

    // character color code from color RAM
    int foregroundColorCode = m_colorLine[charColumn];
    uint8_t foregroundColor = RAWCOLORS[foregroundColorCode & 7];

    // character pixels from char table
    int cv = m_machine->vicBusRead(CHARTABLE_VIC2CPU(m_charTableAddr + charIndex * m_charHeight + m_inCharRow));
    ...
    if (foregroundColorCode & 0x8) {
      ...
      const uint8_t colors[4] = { m_backgroundColor, m_borderColor, foregroundColor, m_auxColor };

      // packs 4 pixels in raw 32 bit word
      uint32_t px =   (colors[(cv >> 2) & 3] << 16)
                    | (colors[(cv >> 2) & 3] << 24)
                    | (colors[(cv >> 0) & 3]      )
                    | (colors[(cv >> 0) & 3] << 8 );
      SETPIXEL4(x, px);
      ...
      const uint8_t colors[2] = { m_backgroundColor, foregroundColor };

      // packs 4 pixels in raw 32 bit word
      uint32_t px =   (colors[(cv >> 3) & 1] << 16)
                    | (colors[(cv >> 2) & 1] << 24)
                    | (colors[(cv >> 1) & 1]      )
                    | (colors[(cv)      & 1] << 8 );
      SETPIXEL4(x, px);
  ...
  if (m_regs[reg] != value) {
    ...
    switch (reg) {
      ...
      case 0xf:
      {
        ...
        m_backgroundColor = RAWCOLORS[backColorCode];
        m_charInvertMask  = (m_regs[0xf] & 0x8) == 0 ? 0xff : 0x00;
        m_borderColor     = RAWCOLORS[m_regs[0xf] & 7];
        m_borderColor4    = m_borderColor | (m_borderColor << 8) | (m_borderColor << 16) | (m_borderColor << 24);
        break;
      }
    }
  }
